...
. Finally, activate the mod in the left pane and we are good to go. This is a FOSE-only mod, so there's no need to rearrange the loar order. Lauch the game and strafe your way around for a while to see for yourself the smoothness the game has aquired.
+
...
. Switch to the *Category* tab and remove the *Stealth* category from the mod.
+
