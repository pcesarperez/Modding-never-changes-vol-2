...
image::DEFCON-00-Mod-Organizer-creating-xEdit-executable-entry.png[xEdit executable information in Mod Organizer]
. The executables settings window remains open, so click on *Close* after checking that an xEdit entry has been created.
+
...
. Back in the MO main window, head to the executables dropdown and select xEdit.
+
...
. With xEdit selected, click on the *Shortcuts* dropdown and select *Toolbar* to create a shortcup on the top right button bar.
+
