...
[WARNING]
====
You *DON'T* need to generate a merged patch for each major DEFCON chapter. You only need to generate a merged patch at the last major DEFCON chapter you decide to use.

For example, if you just want to play the fixed game with a Vanilla experience, you need to create the merged patch (or use the provided one) at the end of the DEFCON 01 chapter. If you want the user interface tweaks and nothing beyond, generate (or download) the merged patch at the end of the DEFCON 02 chapter. And so on, and so forth.

Whatever you do, you need just *ONE* merged patch.

That said, if you intend to continue to the <<DEFCON 02 (User Interface), user interface chapter>>, jump to it and come back when you feel you don't want to install any further mods.
====
...
[WARNING]
====
Like with the merged patches, you *DON'T* need to generate a bashed patch for each major DEFCON chapter. You only need to generate a bashed patch at the last major DEFCON chapter you decide to use.

For example, if you just want to play the fixed game with a Vanilla experience, you need to create the bashed patch (or use the provided one) at the end of the DEFCON 01 chapter. If you want the user interface tweaks and nothing beyond, generate (or download) the bashed patch at the end of the DEFCON 02 chapter. And so on, and so forth.

Whatever you do, you need just *ONE* bashed patch.

That said, if you intend to continue to the <<DEFCON 02 (User Interface), user interface chapter>>, jump to it and come back when you feel you don't want to install any further mods.
====


Take into account that creating a bashed patch is way more complicated than creating a merged patch. Thus, I heartedly recommend using my own pre-generated bashed patches, although, again, you will be able to use them if and only if you have installed each and every one mod in this guide, and in the right order. This was of paramount importance regarding merged patches, and it's even *more* important regarding bashed patches.
...
I will explain how to create a bashed patch only once, in case you need to consult it. At the end of each major DEFCON chapter I will provide a pre-generated bashed patch, locked and loaded, although a link will to the manual creation procedure will be provided, should you need it.




==== Generating your own bashed patch
...
In fact, we should have added the missing bash tags by now, but I thought it would be a little awkward to add them with no clear purpose, under the promise that they would be used in this final sub-chapter.

So, we are going to add bash tags in the following procedure, but from now on, bash tags are going to be added on the go, so that you can use the pre-generated bashed patch at the end of each major DEFCON chapter.
...
. Rename the copy to `DEFCON 01 Bashed Patch.esp`.
...
+
.DEFCON 01 Bashed Patch Quick Install window
image::DEFCON-01-Mod-Organizer-bashed-patch-Quick-Install.png[DEFCON 01 Bashed Patch Quick Install window, title="DEFCON 01 Bashed Patch Quick Install window"]
...
+
.DEFCON 01 Bashed Patch is installed but not active yet
image::DEFCON-01-Mod-Organizer-bashed-patch-installed-but-not-active.png[DEFCON 01 Bashed Patch is installed but not active yet, title="DEFCON 01 Bashed Patch is installed but not active yet"]
...
+
.Correcting the version number of DEFCON 01 Bashed Patch
image::DEFCON-01-Mod-Organizer-bashed-patch-corrected-version.png[Correcting the version number of DEFCON 01 Bashed Patch, title="Correcting the version number of DEFCON 01 Bashed Patch"]
...
+
.DEFCON 01 Bashed Patch is installed and active
image::DEFCON-01-Mod-Organizer-bashed-patch-installed-and-active.png[DEFCON 01 Bashed Patch is installed and active, title="DEFCON 01 Bashed Patch is installed and active"]
...
image::DEFCON-01-Mod-Organizer-LOOT-toolbar-button.png[Launching LOOT through the toolbar button, title="Launching LOOT through the toolbar button"]
. LOOT uses what is called a https://github.com/loot/fallout3/blob/v0.10/masterlist.yaml[masterlist] to keep track of all the integrity details of any conceivable plugin you could have in your stack, including bash tags. Take a look, for example, to what LOOT have to say about the plugins `YeralingBooks.esp`, `Confessor Cromwell has a Break_EV.esp` and `BetterGamePerformance.esp`.
+
.LOOT shows the bash tags of any plugin it knows of
image::DEFCON-01-LOOT-shows-bash-tags-through-masterlist.png[LOOT shows the bash tags of any plugin it knows of, title="LOOT shows the bash tags of any plugin it knows of"]
. You need to write down the bash tags for your entire mod stack (this is something we will do on the go in later chapters). This is the list of bash tags LOOT extracts at this point:
* `BrokenSteel.esm`: `Actors.ACBS, Actors.AIPackages, C.Water, Delev, Factions, Stats`.
* `Unofficial Fallout 3 Patch.esm`: `C.Music, Deflst, Delev, Factions, Invent, Names, Relations, Relev, Stats`.
* `YearlingBooks.esp`: `Scripts`.
* `Confessor Cromwell has a Break_EV.esp`: `Actors.AIPackages`.
* `BetterGamePerformance.esp`: `Deflst, Delev`.
...
image::DEFCON-01-Mod-Organizer-Wrye-Flash-toolbar-button.png[Launching Wrye Flash through the toolbar button, title="Launching Wrye Flash through the toolbar button"]
. Wrye Flash is a strange beast, but we will try to keep the fuss to a minimum. In the *Mods* tab, which should be selected by default, select `BrokenSteel.esm` to show its data in the right column.
+
.Wrye Flash with 'BrokenSteel.esm' selected
image::DEFCON-01-Wrye-Flash-with-BrokenSteel-esm-selected.png[Wrye Flash with 'BrokenSteel.esm' selected, title="Wrye Flash with 'BrokenSteel.esm' selected"]
...
image::DEFCON-01-Wrye-Flash-selecting-bash-tags.png[Selecting bash tags in Wrye Flash, title="Selecting bash tags in Wrye Flash"]
. Repeat the process for the rest of the plugins with bash tags in our list (in fact, you will only have to add `Scripts` to `YearlingBooks.esp`; the rest is identical).
...
image::DEFCON-01-Wrye-Flash-rebuild-patch-option.png[Option to rebuild a bashed patch in Wrye Flash, title="Option to rebuild a bashed patch in Wrye Flash"]
. Every time you rebuild a bashed patch, Wrye Flash detects if there are mods which could be deactivated because they will be, in fact, included in the bashed patch. As a rule of thumb, it is advisable to click on *Yes*, but in this stage, the only mod it suggests deactivating is our merged patch, so click on *No*.
+
.Wrye Flash suggests deactivating the merged patch
image::DEFCON-01-Wrye-Flash-deactivate-suggested-mods.png[Wrye Flash suggests deactivating the merged patch, title="Wrye Flash suggests deactivating the merged patch"]
...
image::DEFCON-01-Wrye-Flash-selecting-all-rebuild-configuration-options.png[Selecting all rebuild configuration options in Wrye Flash, title="Selecting all rebuild configuration options in Wrye Flash"]
...
image::DEFCON-01-Wrye-Flash-select-all-weird-error.png[Weird error window when selecting all items in Wrye Flash, title="Weird error window when selecting all items in Wrye Flash"]
...
image::DEFCON-01-Wrye-Flash-unselecting-unwanted-configuration-items.png[Deselecting unwanted configuration items, title="Deselecting unwanted configuration items"]
. We have to fine-tune certain elements. All sub-items in each category should be checked, except:
* The file `DEFCON 01 Merged Patch.esp` in *Merge Patches*.
* The file `Fallout3.esm` in *Import Names*.
+
.Merged patch not checked in 'Merge Patches' option
image::DEFCON-01-Wrye-Flash-merge-patches-option.png[Merged patch not checked in 'Merge Patches' option, title="Merged patch not checked in 'Merge Patches' option"]
+
.'Fallout3.esm' not checked in 'Import Names' option
image::DEFCON-01-Wrye-Flash-import-names-option.png['Fallout3.esm' not checked in 'Import Names' option, title="'Fallout3.esm' not checked in 'Import Names' option"]
. Additionally, the file `YearlingBooks.esp` must be checked in the *Import Scripts* option.
+
.'YearlingBooks.esp' checked in 'Import Scripts' option
image::DEFCON-01-Wrye-Flash-import-scripts-option.png['YearlingBooks.esp' checked in 'Import Scripts' option, title="'YearlingBooks.esp' checked in 'Import Scripts' option"]
. Press the *Build Patch* button to start building your bashed patch.
+
.It's time to start building the actual bashed patch
image::DEFCON-01-Wrye-Flash-build-patch-button.png[It's time to start building the actual bashed patch, title="It's time to start building the actual bashed patch"]
. Gather your patience, for Wrye Flash will take its time to build the bashed patch.
+
.Wrye Flash is building the bashed patch
image::DEFCON-01-Wrye-Flash-building-bashed-patch.png[Wrye Flash is building the bashed patch, title="Wrye Flash is building the bashed patch"]
...
image::DEFCON-01-Wrye-Flash-bashed-patch-build-report.png[Final report about the building of the bashed patch, title="Final report about the building of the bashed patch"]
...
image::DEFCON-01-Mod-Organizer-bashed-patch-almost-ready.png[The bashed patch is almost ready to fly, title="The bashed patch is almost ready to fly"]
...
image::DEFCON-01-Mod-Organizer-moving-bashed-patch-docs.png[Moving the documentation generated by Wrye Flash to the bashed patch, title="Moving the documentation generated by Wrye Flash to the bashed patch"]
...
image::DEFCON-01-Mod-Organizer-bashed-patch-documentation.png[The bashed patch documentation is in its right place, title="The bashed patch documentation is in its right place"]
...
==== Using a pre-generated bashed patch
